Find files and directories using glob patterns to match on the paths. This tool supports standard glob syntax like `*`, `?`, and `**` for recursive searches.

**When to use:**
- Prefer the `Glob` tool over running `find` or `fd` through the `Bash` tool
